...
class Essay:
    ...
    def get_non_ACS(self, order="reordering", normalised=False):
        """
        Get non-AC sentences
        ...
        Returns:
            list of non-AC sentences
        """
        units_copy = deepcopy(self.units)
        if order == "original":
            units_copy = sorted(units_copy, key=lambda unit: int(unit.ID))

        _, rels = self.get_rel_distances(order, True)

        output = []
        for i in range(len(units_copy)):
            if rels[i] == "non-AC":
                if normalised:
                    output.append(Essay.discard_text_repair(units_copy[i].text))
                else:
                    output.append(Essay.use_text_repair(units_copy[i].text))
        return output



    def text_repair_stats(self):
        """
        Get the number of sentences involved in text repair

        Returns:
            cnt_major_claim     --> number of major claims repaired
            cnt_non_mc          --> number of other sentences involved in repair
        """
        mc = 0
        non_mc = 0

        _, rels = self.get_rel_distances(mode="original", include_non_arg_units=True)
        units_copy = deepcopy(self.units)
        units_copy = sorted(units_copy, key=lambda unit: int(unit.ID)) # original order

        for i in range(len(units_copy)):
            if '[' in units_copy[i].text:
                if rels[i] == "major claim":
                    mc += 1
                else:
                    # print(units_copy[i].text)
                    non_mc += 1
