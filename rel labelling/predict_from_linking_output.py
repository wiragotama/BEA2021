...
def get_dataset_reader():
    ...
    config = Config(
        ...
        batch_size=1 # for testing, this does not matter
    )
...
def load_BERT_run(run_dir):
    ...
    print("loading BERT model", run_dir)
    model = load_model(run_dir, torch_device)
    print("Model architecture:", model.param["class"])
    print("Model params", model.param)
    model.to(torch_device)

    return cuda_device, torch_device, model
...
def save_tsv(save_dir, essay_code, sentences, dist, label_preds):
    ...
    # save filename
    filename = save_dir + "/" + essay_code + ".tsv"
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        '-pred_dir', '--pred_dir', type=str, help='directory containing the prediction result', required=True)
    parser.add_argument(
        '-model_dir', '--model_dir', type=str, help='directory containing the labelling model', required=True)
    parser.add_argument(
    ...
    for pred_dir in pred_dirs:
        print("Open prediction in", pred_dir)
        with open(pred_dir+"/link_preds.txt") as f:
            content = f.readlines()[0]
            prediction_dist = ast.literal_eval(content)

        curr_run = pred_dir.split("/")[-1]

        # load model
        if os.path.exists(args.model_dir+"curr_run"+"/"):
            cuda_device, torch_device, model = load_BERT_run(args.model_dir+curr_run+"/")
            ...
            for i in range(len(test_essays)):
                ...
                # save to file
                save_tsv(args.out_dir+curr_run +"/", test_essays[i].essay_code, sentences, dist, label_preds)
                # input()
            ...
            print("BERT model for %s does not exist" % (curr_run))
